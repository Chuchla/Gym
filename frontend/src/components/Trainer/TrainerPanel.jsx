    ...
    const results = events
      ...
      .filter((ev) => {
        ...
        const matchUser = ev.clients?.some((client) =>
          `${client.first_name} ${client.last_name}`.toLowerCase().includes(searchUser.toLowerCase())
        );
        return (searchName ? matchName : true) && (searchUser ? matchUser : true);
      });
      ...
      <div className="search-panel">
        ...
        <div className="search-fields">
          ...
          <input
            ...
            className="search-input"
          />
          <button onClick={handleSearch} className="add-event-btn">Search</button>
        </div>
              ...
              >
                ...
                {event.clients && event.clients.length > 0 && (
                  <div style={{ fontSize: "14px", marginTop: "4px" }}>
                    Clients: {event.clients.map(c => `${c.first_name} ${c.last_name}`).join(", ")}
                  </div>
                )}
              </li>
            ))}
